            ...
            if (this.readyState == 4) {
                if (this.status == 200) {
                    ...
                    for (var a = 0; a < data.length; a++) {
                        self.renderMessage(data[a]);
                    }

                    if (self.chatUser != null) {
                        var form = document.getElementById("form-initialize-chat-widget");
    ...
    initializeChat: function (form) {
        ...
        this.getIpAddress(function () {
            ...
            self.socketIO.on("user_new_message", function (data) {
                ...
                var selector = document.querySelector("#chat-widget form input.message");
        ...
        ajax.onreadystatechange = function () {
            if (this.readyState == 4) {
                if (this.status == 200) {
                    ...
                    var partsIp = response.split("ip=");
                    if (partsIp.length > 1) {
                        var partsTs = partsIp[1].split("ts");
                        if (partsTs.length > 0) {
                            self.ipAddress = partsTs[0].trim();
                        }
                    }

                    var partsUag = response.split("uag=");
                    if (partsUag.length > 1) {
                        var partsColo = partsUag[1].split("colo");
                        if (partsColo.length > 0) {
                            self.browserInfo = partsColo[0].trim();
                        }
                    }

                    var partsLoc = response.split("loc=");
                    if (partsLoc.length > 1) {
                        var partsTls = partsLoc[1].split("tls");
